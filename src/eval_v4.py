...
def main():
    ...
    mesh_dir = os.path.join(run_dir, 'meshes', args.split) if args.save_meshes else None

    print(f"\nRunning evaluation...")
    results = evaluate(
        model, eval_loader, device,
        save_meshes=args.save_meshes, mesh_dir=mesh_dir,
        faces=faces, template_verts=template_verts, n_save=3,
    )
